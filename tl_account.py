...
for index, arg in enumerate(sys.argv):
    if index == 1:
        mbots_uniq_id = arg                                                                         elif index == 2:
        message_id = int(arg)
    elif index == 3:
        type_ = arg
...
cs.execute(f"SELECT * FROM {utl.admin}")                                                        row_admin = cs.fetchone()                                                                       
cs.execute(f"SELECT * FROM {utl.mbots} WHERE uniq_id='{mbots_uniq_id}'")                        row_mbots = cs.fetchone()
